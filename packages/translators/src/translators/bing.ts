...
class BingTranslator {
    ...
    async updateTokens() {
        ...
        const finalURL =
            (response as any)?.request?.responseURL // 真实 axios（XHR）分支
            ?? (response as any)?.url                // fetch 分支
            ?? "";

        const responseHost = finalURL
            ? /(https:\/\/.*\.bing\.com\/).*/g.exec(finalURL)
            : null;
